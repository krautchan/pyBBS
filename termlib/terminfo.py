...
class TermInfo(object):
    ...
    def __init__(self, term):
        ...
        if os.path.exists(os.path.join('/','lib','terminfo',term[0],term)):
            ...
            print 'terminfo for \'%s\' not found' % term
            self.term = 'vt100'
            self.loadTermInfo()
    ...
    def tigets(self,cap,*args):
        ...
        while sp < len(str) and str[sp]:
            ...
            if str[sp] != '%':
                ...
                if m:
                    stack.append(p[int(m.group('pn'))-1])
                    sp += len(m.group())
    ...
    def getAlternateChar(self,char):
        list = {}
        chars = self._getStrCap('acsc')
        for i in range(0,len(chars)/2):
            list[chars[i*2]] = chars[i*2+1]
        return list[char]
    ...
    def putQuirks(self):
        if self.term[0:5] == 'xterm':
            self.putQuirksXTerm()
            if self.term == 'xterm-256color':
                self.putQuirksXTerm256Color()
    def putQuirksXTerm256Color(self):
        if self.strcaps.index('setb') < len(self.strvals):
            self.strvals[self.strcaps.index('setb')] = self._getStrCap('setab')
        if self.strcaps.index('setf') < len(self.strvals):
            self.strvals[self.strcaps.index('setf')] = self._getStrCap('setaf')
    def putQuirksXTerm(self):
        if self.strcaps.index('kcud1') < len(self.strvals):
            self.strvals[self.strcaps.index('kcud1')] = '\x1b[B'
        if self.strcaps.index('kcuf1') < len(self.strvals):
            self.strvals[self.strcaps.index('kcuf1')] = '\x1b[C'
        if self.strcaps.index('kcub1') < len(self.strvals):
            self.strvals[self.strcaps.index('kcub1')] = '\x1b[D'
        if self.strcaps.index('kcuu1') < len(self.strvals):
            self.strvals[self.strcaps.index('kcuu1')] = '\x1b[A'